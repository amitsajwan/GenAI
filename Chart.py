...
elements += [{"data": {"id": f"{edge['from']}->{edge['to']}", "source": edge["from"], "target": edge["to"]}} for edge in data["edges"]]
...
app.layout = html.Div([
    ...
    cyto.Cytoscape(
        ...
        stylesheet=[
            ...
            {"selector": ":selected", "style": {"line-color": "blue", "border-width": 3}},  # Highlight selection
        ],
    ),
...
def save_execution_order(n_clicks, elements):
    """Save execution order after modifying edges."""
    if n_clicks > 0:
        new_edges = [
