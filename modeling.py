...
* Define a function `train_and_tune_model(X_train_processed, y_train_raw)`:
    ...
    * **[Prompt Creator: Insert the Python dictionary for `param_grid` from your Phase 2C decision. E.g., `param_grid = {{ 'n_estimators': [75, 100, 150], 'max_depth': [7, 10, 15, 20], ... }}`]**
    * Instantiate `GridSearchCV` (or `RandomizedSearchCV` if decided) with the model, `param_grid`, `cv=3` (or 5), `scoring='neg_root_mean_squared_error'` (or 'r2', as decided), and `n_jobs=-1`.
    * Fit the `GridSearchCV` object on `X_train_processed` and `y_train_raw.values.ravel()`.
    * Extract the best estimator: `final_model = grid_search.best_estimator_`.
    * Print the best parameters: `print(f"Best hyperparameters found: {{grid_search.best_params_}}")`.
    * Returns `final_model` and `grid_search.best_params_`.
* Call this function.

**4. Model Evaluation Function:**
* Define a function `evaluate_model(model, X_data, y_data, dataset_name)`:
    * Makes predictions: `y_pred = model.predict(X_data)`.
    * Calculates MAE, MSE, RMSE (`np.sqrt(MSE)`), and R².
    * Prints metrics: `print(f"{{dataset_name}} Metrics: MAE: {{mae:.4f}}, MSE: {{mse:.4f}}, RMSE: {{rmse:.4f}}, R2: {{r2:.4f}}")`.
    * Returns a dictionary of these metrics.
* Call this function for the `final_model` on:
    * Training data (X_train_processed, y_train)
    * Validation data (X_val_processed, y_val)
    * Test data (X_test_processed, y_test)
* Store the returned metrics dictionaries.

**5. Save Model Function:**
* Define a function `save_model_artifact(model, path)`:
    * Saves the `model` to `path` using `joblib.dump()`.
    * `print(f"Model saved to: {{path}}")`.
* Call this to save `final_model` to `model_output_path_script`.

**6. Prepare and Save Evaluation Report Function:**
* Define a function `generate_report(report_path, chosen_model_name, best_params, train_metrics, val_metrics, test_metrics)`:
    * Opens `report_path` in write mode.
    * Writes the following information:
        * Chosen Model: `chosen_model_name` **[Prompt Creator: This will be the string name of your model from Phase 2B]**
        * Best Hyperparameters: `str(best_params)`
        * Performance Metrics (formatted nicely):
            * Training Set: MAE, MSE, RMSE, R² from `train_metrics`
            * Validation Set: MAE, MSE, RMSE, R² from `val_metrics`
            * Test Set: MAE, MSE, RMSE, R² from `test_metrics`
        * Placeholder for Observations: "Observations and Insights:\n\t- Overfitting/Underfitting Analysis: [Compare train/val/test R2 scores. High train R2 but significantly lower val/test R2 suggests overfitting.]\n\t- Model Robustness: [Compare val/test R2 scores. Similar scores suggest good generalization from validation to test.]\n\t- Further Suggestions: [Consider more feature engineering, different algorithms, or more data if performance is unsatisfactory.]"
    * `print(f"Evaluation report saved to: {{report_path}}")`.
* Call this function, passing the necessary information.

**7. Main Script Execution Flow (Wrap calls to above functions in a `if __name__ == '__main__':` block):**
* Load data.
* Preprocess data.
...
* Print a final success message: `print("Modeling script execution completed successfully.")`
